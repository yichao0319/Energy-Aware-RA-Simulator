...
class EngTputRA(BaseRA):
    ...
    def calc_energy(self):
        ...
        for key in keys:
            ...
            #self.est_energy[key] = [energyfnc(nant,nss,plen,mcs,self.per[key][mcs]) for mcs in range(0,8)]
            #print "key: "+str(key)+" -> "+str(self.est_energy[key])
...
#                if self.tput[key][mcs] >= tput_th and self.est_energy[key][mcs] < min_energy:
    ...
    def select_rate(self,pkt):
        ...
        trace = {'mcs':rate_sel['mcs'],'mode':rate_sel['key'],'tput':rate_sel['tput'],'energy':rate_sel['eng'],'succ':pkt.succ}
        print trace
